                ...
                <tbody>
                  {filtered.map((lead) => (
                    <tr key={lead.id}>
                      ...
                      <td>{lead.website ? <span className="signal good">● Есть сайт</span> : <span className="signal pending">● Сайт не проверен</span>}{lead.rating != null && <div className="muted" style={{ marginTop: 6 }}>★ {lead.rating.toFixed(1)}</div>}</td>
